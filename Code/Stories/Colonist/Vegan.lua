...
local function CheckStory2()
	...
	if not Sent[Key2] and Published[Key1] and CountColonistsWithTrait(TraitId) > 0
	then
		local MartianTribuneMod = MartianTribuneMod
		...
		local index, Vegan1Story = FindStoryInListByKey(SocialArchive, Key1)

		if Vegan1Story and UICity.day >= (Vegan1Story.publishedDay + 10) then
			local AddStory = MartianTribuneMod.Functions.AddSocialPotentialStory
			local VeganColonistFallbackName = MartianTribuneMod.Name.VeganColonist
			-- local IsValidColonist = MartianTribuneMod.Functions.IsValidColonist
...
local function CheckStory3()
	...
	then
		...
		local FindStoryInListByKey = MartianTribuneMod.Functions.FindStoryInListByKey
		local SocialArchive = MartianTribune.SocialArchive
		local index, Vegan2Story = FindStoryInListByKey(SocialArchive, Key2)
		if Vegan2Story and UICity.day >= (Vegan2Story.publishedDay + 11) then
			local AddStory = MartianTribuneMod.Functions.AddSocialPotentialStory
			AddStory({
				key = Key3,
...
local function CheckStory4()
	...
	then
		...
		local FindStoryInListByKey = MartianTribuneMod.Functions.FindStoryInListByKey
		local SocialArchive = MartianTribune.SocialArchive
		local index, Vegan3Story = FindStoryInListByKey(SocialArchive, "Vegan3")
		if Vegan3Story and UICity.day >= (Vegan3Story.publishedDay + 14) then
			local LeaderTitle = MartianTribune.LeaderTitle
